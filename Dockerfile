...
if [ -n "$HOME" ] && [ -d "$HOME" ]; then
    ...
    for jb_base in "${JB_BASE_DIRS[@]}"; do
        ...
        for idea_dir in "${IDEA_DIRS[@]}"; do
            ...
            # 1. Disable GTK native file chooser in idea.properties to prevent file dialog freezes in WSLg
            props_file="$idea_dir/idea.properties"
            if [ ! -f "$props_file" ]; then
                echo "ide.use.native.file.chooser=false" > "$props_file"
            elif ! grep -q "ide.use.native.file.chooser" "$props_file"; then
                echo "ide.use.native.file.chooser=false" >> "$props_file"
            fi

            # 2. MCP Server port 64343 in mcpServer.xml
...
XML_EOF
            ...
            # 3. Noto Color Emoji fallback font in editor.font.xml
